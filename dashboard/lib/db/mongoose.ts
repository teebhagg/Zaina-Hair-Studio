...
// This allows dotenv to load it before this module is evaluated
function getMongoUri() {
  let uri = process.env.MONGODB_URI || "";

  // If MONGODB_DATABASE is specified, ensure it's in the URI
  const databaseName = process.env.MONGODB_DATABASE || "zainab-salon";
  ...
  if (queryIndex === -1) {
    ...
    const baseUri = beforeQuery.endsWith("/")
...
async function connectDB() {
  ...
  if (!MONGODB_URI) {
    if (process.env.NODE_ENV !== "production") {
      console.warn(
        "Warning: MONGODB_URI is not set. Database features will not work."
      );
    }
    throw new Error(
